...
contract SetUp is BaseTest {
    ...
    bytes internal constant offchainConfigBytes =
        abi.encode(1234, ZERO_ADDRESS);
    ...
    function setUp() public virtual override {
        super.setUp();
        (registry, ) = deployAndConfigureRegistryAndRegistrar(
            AutoBase.PayoutMode.ON_CHAIN
        );
        ...
        registry.addFunds(
            linkUpkeepID,
            registry.getMinBalanceForUpkeep(linkUpkeepID)
        );
        registry.addFunds(
            linkUpkeepID2,
            registry.getMinBalanceForUpkeep(linkUpkeepID2)
        );
        registry.addFunds(
            usdUpkeepID18,
            registry.getMinBalanceForUpkeep(usdUpkeepID18)
        );
        registry.addFunds(
            usdUpkeepID6,
            registry.getMinBalanceForUpkeep(usdUpkeepID6)
        );
        registry.addFunds(
            nativeUpkeepID,
            registry.getMinBalanceForUpkeep(nativeUpkeepID)
        );
        vm.stopPrank();
    }
}
...
contract CheckUpkeep is SetUp {
    function testPreventExecutionOnCheckUpkeep() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(Registry.OnlySimulatedBackend.selector)
        );
...
contract WithdrawFunds is SetUp {
    ...
    function test_Happy_Link() external {
        ...
        uint256 startLinkReserveAmountBalance = registry.getReserveAmount(
            address(linkToken)
        );
        ...
        assertEq(
            linkToken.balanceOf(UPKEEP_ADMIN),
            startUpkeepAdminBalance + upkeepBalance
        );
        assertEq(
            registry.getReserveAmount(address(linkToken)),
            startLinkReserveAmountBalance - upkeepBalance
        );
    }
    ...
    function test_Happy_USDToken() external {
        ...
        uint256 startUSDToken6ReserveAmountBalance = registry.getReserveAmount(
            address(usdToken6)
        );
        ...
        assertEq(
            usdToken6.balanceOf(UPKEEP_ADMIN),
            startUpkeepAdminBalance + upkeepBalance
        );
        assertEq(
            registry.getReserveAmount(address(usdToken6)),
            startUSDToken6ReserveAmountBalance - upkeepBalance
        );
    }
}
...
contract AddFunds is SetUp {
    ...
    function test_movesFundFromCorrectToken() public {
        ...
        uint256 startLINKRegistryBalance = linkToken.balanceOf(
            address(registry)
        );
        uint256 startUSDRegistryBalance = usdToken18.balanceOf(
            address(registry)
        );
        ...
        assertEq(
            linkToken.balanceOf(address(registry)),
            startLINKRegistryBalance + 1
        );
        assertEq(
            usdToken18.balanceOf(address(registry)),
            startUSDUpkeepBalance
        );

        registry.addFunds(usdUpkeepID18, 2);
        assertEq(registry.getBalance(linkUpkeepID), startLinkUpkeepBalance + 1);
        assertEq(
            registry.getBalance(usdUpkeepID18),
            startUSDRegistryBalance + 2
        );
        assertEq(
            linkToken.balanceOf(address(registry)),
            startLINKRegistryBalance + 1
...
contract Withdraw is SetUp {
    ...
    function testLinkAvailableForPaymentReturnsLinkBalance() public {
        ...
        assertEq(
            uint256(registry.linkAvailableForPayment()),
            uint256(startLinkAvailable) + 1e10
        );
    }

    function testWithdrawLinkRevertsBecauseOnlyFinanceAdminAllowed() public {
        vm.expectRevert(
            abi.encodeWithSelector(Registry.OnlyFinanceAdmin.selector)
        );
    ...
    function testWithdrawLinkRevertsBecauseOfInsufficientBalance() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(Registry.InsufficientBalance.selector, 0, 1)
        );
        registry.withdrawLink(aMockAddress, 1);
    ...
    function testWithdrawLinkRevertsBecauseOfInvalidRecipient() public {
        ...
        // try to withdraw 1 link while there is 0 balance
        vm.expectRevert(
            abi.encodeWithSelector(Registry.InvalidRecipient.selector)
        );
    ...
    function test_WithdrawERC20Fees_RespectsReserveAmount() public {
        assertEq(
            registry.getBalance(usdUpkeepID18),
            registry.getReserveAmount(address(usdToken18))
        );
        vm.startPrank(FINANCE_ADMIN);
        vm.expectRevert(
            abi.encodeWithSelector(Registry.InsufficientBalance.selector, 0, 1)
    ...
    function test_WithdrawERC20Fees_RevertsWhen_AttemptingToWithdrawLINK()
        public
    {
        _mintLink(address(registry), 1e10);
        vm.startPrank(FINANCE_ADMIN);
    ...
    function test_WithdrawERC20Fees_RevertsWhen_LinkAvailableForPaymentIsNegative()
        public
    {
        _transmit(usdUpkeepID18, registry); // adds USD token to finance withdrawable, and gives NOPs a LINK balance
        require(
            registry.linkAvailableForPayment() < 0,
            "linkAvailableForPayment should be negative"
        );
        require(
        ...
        _mintLink(
            address(registry),
            uint256(registry.linkAvailableForPayment() * -10)
        ); // top up LINK liquidity pool
        vm.prank(FINANCE_ADMIN);
    ...
    function test_WithdrawERC20Fees_InOffChainMode_Happy() public {
        ...
        (Registry registry, ) = deployAndConfigureRegistryAndRegistrar(
...
contract SetConfig is SetUp {
    ...
    function testSetConfigSuccess() public {
        (uint32 configCount, uint32 blockNumber, ) = registry
            .latestConfigDetails();
        assertEq(configCount, 1);
        ...
        AutomationRegistryBase2_3.BillingConfig[]
            memory billingConfigs = new AutomationRegistryBase2_3.BillingConfig[](
                1
            );
        ...
        bytes memory onchainConfigBytesWithBilling = abi.encode(
            cfg,
            billingTokens,
            billingConfigs
        );
        ...
        AutomationRegistryBase2_3.BillingConfig memory config = registry
            .getBillingTokenConfig(billingTokenAddress);
    ...
    function testSetConfigMultipleBillingConfigsSuccess() public {
        ...
        AutomationRegistryBase2_3.BillingConfig memory config1 = registry
            .getBillingTokenConfig(billingTokenAddress1);
        ...
        AutomationRegistryBase2_3.BillingConfig memory config2 = registry
            .getBillingTokenConfig(billingTokenAddress2);
    ...
    function testSetConfigTwiceAndLastSetOverwrites() public {
        ...
        AutomationRegistryBase2_3.BillingConfig[]
            memory billingConfigs1 = new AutomationRegistryBase2_3.BillingConfig[](
                1
            );
        ...
        bytes memory onchainConfigBytesWithBilling1 = abi.encode(
            cfg,
            billingTokens1,
            billingConfigs1
        );
        ...
        (
            ,
            IAutomationV21PlusCommon.OnchainConfigLegacy memory onchainConfig1,
            ,
            ,
        ...
        AutomationRegistryBase2_3.BillingConfig[]
            memory billingConfigs2 = new AutomationRegistryBase2_3.BillingConfig[](
                1
            );
        ...
        newRegistrars[0] = address(
            uint160(uint256(keccak256("newRegistrar1")))
        );
        newRegistrars[1] = address(
            uint160(uint256(keccak256("newRegistrar2")))
        );
        newRegistrars[2] = address(
            uint160(uint256(keccak256("newRegistrar3")))
        );

        // new onchain config with 3 new registrars, all other fields stay the same as the default
        AutomationRegistryBase2_3.OnchainConfig
            memory cfg2 = AutomationRegistryBase2_3.OnchainConfig({
                checkGasLimit: 5_000_000,
                stalenessSeconds: 90_000,
                gasCeilingMultiplier: 0,
                maxPerformGas: 10_000_000,
        ...
        bytes memory onchainConfigBytesWithBilling2 = abi.encode(
            cfg2,
            billingTokens2,
            billingConfigs2
        );
    ...
    function testSetConfigDuplicateBillingConfigFailure() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(Registry.DuplicateEntry.selector)
        );
        registry.setConfig(
            SIGNERS,
            TRANSMITTERS,
    ...
    function testSetConfigOnTransmittersAndPayees() public {
        ...
        AutomationRegistryBase2_3.TransmitterPayeeInfo[]
            memory transmitterPayeeInfos = registry.getTransmittersWithPayees();
        assertEq(transmitterPayeeInfos.length, TRANSMITTERS.length);

        for (uint256 i = 0; i < transmitterPayeeInfos.length; i++) {
            address transmitterAddress = transmitterPayeeInfos[i]
                .transmitterAddress;
...
contract NOPsSettlement is SetUp {
    ...
    function testSettleNOPsOffchainRevertDueToOffchainSettlementDisabled()
        public
    {
        (Registry registry, ) = deployAndConfigureRegistryAndRegistrar(
            AutoBase.PayoutMode.OFF_CHAIN
        ...
        vm.prank(FINANCE_ADMIN);
        vm.expectRevert(
            abi.encodeWithSelector(Registry.MustSettleOnchain.selector)
        );
        registry.settleNOPsOffchain();
    }
    ...
    function testSettleNOPsOffchainSuccessWithERC20MultiSteps() public {
        ...
        for (uint256 i = 0; i < TRANSMITTERS.length; i++) {
            (bool active, uint8 index, uint96 balance, , ) = registry
                .getTransmitterInfo(TRANSMITTERS[i]);
            assertTrue(active);
            assertEq(i, index);
            assertEq(0, balance);
        }
        ...
        uint256 erc20ForPayment1 = registry.getAvailableERC20ForPayment(
            address(usdToken18)
        );
        require(
            erc20ForPayment1 > 0,
        ...
        uint256 erc20ForPayment2 = registry.getAvailableERC20ForPayment(
            address(usdToken18)
        );
        require(
            erc20ForPayment2 > erc20ForPayment1,
            "ERC20AvailableForPayment should be greater after another transmit"
        );

        // finance admin comes to withdraw all available ERC20s
        vm.startPrank(FINANCE_ADMIN);
        registry.withdrawERC20Fees(
            address(usdToken18),
            FINANCE_ADMIN,
            erc20ForPayment2
        );

        uint256 erc20ForPayment3 = registry.getAvailableERC20ForPayment(
            address(usdToken18)
        );
        require(
            erc20ForPayment3 == 0,
            "ERC20AvailableForPayment should be 0 now after withdrawal"
        );

        uint256 reservedLink = registry.getReserveAmount(address(linkToken));
        require(
            reservedLink > 0,
            "Reserve amount of LINK should be positive since there was another transmit"
        );
        ...
        vm.startPrank(FINANCE_ADMIN);
        uint256 erc20ForPayment4 = registry.getAvailableERC20ForPayment(
            address(usdToken18)
        );
        vm.expectRevert(
            abi.encodeWithSelector(Registry.InsufficientLinkLiquidity.selector)
        );
        registry.withdrawERC20Fees(
            address(usdToken18),
            FINANCE_ADMIN,
    ...
    function testSettleNOPsOffchainForDeactivatedTransmittersSuccess() public {
        ...
        (
            Registry registry,
            Registrar registrar
        ) = deployAndConfigureRegistryAndRegistrar(
                AutoBase.PayoutMode.OFF_CHAIN
            );
        ...
        assertEq(
            l.topics[0],
            keccak256("NOPsSettledOffchain(address[],uint256[])")
        );
        (address[] memory actualPayees, uint256[] memory actualPayments) = abi
            .decode(l.data, (address[], uint256[]));
        ...
        for (uint256 i = 0; i < NEW_TRANSMITTERS.length; i++) {
            (bool active, uint8 index, uint96 balance, , ) = registry
                .getTransmitterInfo(NEW_TRANSMITTERS[i]);
            assertTrue(active);
            assertEq(i, index);
            assertEq(0, balance);
        }
    ...
    function testSinglePerformAndNodesCanWithdrawOnchain() public {
        ...
        for (uint256 i = 0; i < TRANSMITTERS.length; i++) {
            (, , uint96 balance, , address payee) = registry.getTransmitterInfo(
                TRANSMITTERS[i]
            );
            vm.prank(payee);
            vm.expectEmit();
    ...
    function _configureWithNewTransmitters(
        Registry registry,
        Registrar registrar
    ) internal {
        ...
        uint256[] memory minRegistrationFees = new uint256[](
            billingTokens.length
        );
        minRegistrationFees[0] = 100e18; // 100 USD

        address[] memory billingTokenAddresses = new address[](
            billingTokens.length
        );
        ...
        AutomationRegistryBase2_3.BillingConfig[]
            memory billingTokenConfigs = new AutomationRegistryBase2_3.BillingConfig[](
                billingTokens.length
            );
        ...
        AutomationRegistryBase2_3.OnchainConfig
            memory cfg = AutomationRegistryBase2_3.OnchainConfig({
                checkGasLimit: 5_000_000,
                stalenessSeconds: 90_000,
...
contract WithdrawPayment is SetUp {
    function testWithdrawPaymentRevertDueToOffchainPayoutMode() public {
        registry = deployRegistry(AutoBase.PayoutMode.OFF_CHAIN);
        vm.expectRevert(
            abi.encodeWithSelector(Registry.MustSettleOffchain.selector)
        );
...
contract OnTokenTransfer is SetUp {
    ...
    function test_RevertsWhen_TheUpkeepDoesNotUseLINKAsItsBillingToken()
        public
    {
        vm.startPrank(address(linkToken));
        vm.expectRevert(Registry.InvalidToken.selector);
...
contract GetMinBalanceForUpkeep is SetUp {
    function test_accountsForFlatFee_with18Decimals() public {
        // set fee to 0
        AutomationRegistryBase2_3.BillingConfig memory usdTokenConfig = registry
            .getBillingTokenConfig(address(usdToken18));
        usdTokenConfig.flatFeeMilliCents = 0;
        _updateBillingTokenConfig(
            registry,
            address(usdToken18),
            usdTokenConfig
        );

        uint256 minBalanceBefore = registry.getMinBalanceForUpkeep(
            usdUpkeepID18
        );
        ...
        uint256 minBalanceAfter = registry.getMinBalanceForUpkeep(
            usdUpkeepID18
        );
        assertEq(
            minBalanceAfter,
            minBalanceBefore +
                ((uint256(usdTokenConfig.flatFeeMilliCents) * 1e13) /
                    10 ** (18 - usdTokenConfig.decimals))
        );
    }
    ...
    function test_accountsForFlatFee_with6Decimals() public {
        // set fee to 0
        AutomationRegistryBase2_3.BillingConfig memory usdTokenConfig = registry
            .getBillingTokenConfig(address(usdToken6));
        usdTokenConfig.flatFeeMilliCents = 0;
...
contract BillingOverrides is SetUp {
    event BillingConfigOverridden(
        uint256 indexed id,
        AutomationRegistryBase2_3.BillingOverrides overrides
    );
    ...
    function test_RevertsWhen_NotPrivilegeManager() public {
        AutomationRegistryBase2_3.BillingOverrides
            memory billingOverrides = AutomationRegistryBase2_3
                .BillingOverrides({
                    gasFeePPB: 5_000,
                    flatFeeMilliCents: 20_000
                });
    ...
    function test_Happy_MaxGasPayment_WithBillingOverrides() public {
        uint96 maxPayment1 = registry.getMaxPaymentForGas(
            linkUpkeepID,
            0,
            5_000_000,
            address(linkToken)
        ...
        uint96 maxPayment2 = registry.getMaxPaymentForGas(
            linkUpkeepID,
            0,
            5_000_000,
            address(linkToken)
...
contract Transmit is SetUp {
    function test_transmitRevertWithExtraBytes() external {
        ...
        bytes
            memory exampleRawReport = hex"deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef";

        bytes32[] memory exampleRs = new bytes32[](3);
        exampleRs[0] = bytes32(
            0x1234561234561234561234561234561234561234561234561234561234561234
        );
        exampleRs[1] = bytes32(
            0x1234561234561234561234561234561234561234561234561234561234561234
        );
        exampleRs[2] = bytes32(
            0x7890789078907890789078907890789078907890789078907890789078907890
        );
        ...
        (bool success, bytes memory returnData) = address(registry).call(
            badTransmitData
        ); // send the bogus transmit
        assertFalse(success, "Call did not revert as expected");
        assertEq(
            returnData,
            abi.encodePacked(Registry.InvalidDataLength.selector)
        );
        vm.stopPrank();
    }
    ...
    struct PaymentReceipt {
        ...
        uint96 gasReimbursementETHuels;
        uint96 premiumETHuels;
        IERC20 billingToken;
        uint96 linkUSD;
        uint96 nativeUSD;
        uint96 billingUSD;
    ...
    function test_whenFirstUpkeepFails_subsequentUpkeepsPerform() external {
        ...
        // do the transmit
        vm.expectEmit();
        emit ReorgedUpkeepReport(
            linkUpkeepID,
            abi.encode(block.number, blockhash(block.number))
        );
        vm.expectEmit(true, false, false, false);
        emit UpkeepCharged(
            linkUpkeepID2,
            PaymentReceipt(0, 0, 0, 0, linkToken, 0, 0, 0)
        );
        ...
        require(
            prevUpkeepBalances[0] == registry.getBalance(linkUpkeepID),
            "link upkeep balance should remain the same"
        );
        require(
            prevUpkeepBalances[1] > registry.getBalance(linkUpkeepID2),
            "link upkeep 2 balance should have decreased"
        );
        require(
            prevUpkeepBalances[2] > registry.getBalance(nativeUpkeepID),
            "native upkeep balance should have decreased"
        );
    ...
    function test_handlesMixedBatchOfBillingTokens() external {
        ...
        require(
            registry.getAvailableERC20ForPayment(address(usdToken18)) == 0,
            "ERC20AvailableForPayment should be 0"
        );
        require(
            registry.getAvailableERC20ForPayment(address(weth)) == 0,
            "ERC20AvailableForPayment should be 0"
        );
        ...
        require(
            prevUpkeepBalances[0] > registry.getBalance(linkUpkeepID),
            "link upkeep balance should have decreased"
        );
        require(
            prevUpkeepBalances[1] > registry.getBalance(usdUpkeepID18),
            "usd upkeep balance should have decreased"
        );
        require(
            prevUpkeepBalances[2] > registry.getBalance(nativeUpkeepID),
            "native upkeep balance should have decreased"
    ...
    function test_handlesInsufficientBalanceWithUSDToken18() external {
        ...
        assertEq(
            l1.topics[0],
            keccak256(
                "UpkeepCharged(uint256,(uint96,uint96,uint96,uint96,address,uint96,uint96,uint96))"
            )
        );
        (
            uint96 gasChargeInBillingToken,
        ...
        assertEq(gasReimbursementETHuels, (balance * billingUSD) / linkUSD);
        assertEq(premiumETHuels, 0);
    ...
    function test_handlesInsufficientBalanceWithUSDToken6() external {
        ...
        assertEq(
            premiumETHuels,
            (balance * billingUSD * 1e12) / linkUSD - gasReimbursementETHuels
        ); // scale to 18 decimals
        assertEq(
            premiumInBillingToken,
            (premiumETHuels * linkUSD + (billingUSD * 1e12 - 1)) /
                (billingUSD * 1e12)
        );
    }
}
...
contract MigrateReceive is SetUp {
    ...
    function setUp() public override {
        super.setUp();
        (newRegistry, ) = deployAndConfigureRegistryAndRegistrar(
            AutoBase.PayoutMode.ON_CHAIN
        );
    ...
    function test_Success() external {
        ...
        prevUpkeepTriggerData[0] = registry.getUpkeepTriggerConfig(
            linkUpkeepID
        );
        prevUpkeepTriggerData[1] = registry.getUpkeepTriggerConfig(
            linkUpkeepID2
        );
        prevUpkeepTriggerData[2] = registry.getUpkeepTriggerConfig(
            usdUpkeepID18
        );
        prevUpkeepTriggerData[3] = registry.getUpkeepTriggerConfig(
            nativeUpkeepID
        );
        ...
        emit UpkeepMigrated(
            linkUpkeepID,
            prevUpkeepBalances[0],
            address(newRegistry)
        );
        vm.expectEmit(address(registry));
        emit UpkeepMigrated(
            linkUpkeepID2,
            prevUpkeepBalances[1],
            address(newRegistry)
        );
        vm.expectEmit(address(registry));
        emit UpkeepMigrated(
            usdUpkeepID18,
            prevUpkeepBalances[2],
            address(newRegistry)
        );
        vm.expectEmit(address(registry));
        emit UpkeepMigrated(
            nativeUpkeepID,
            prevUpkeepBalances[3],
            address(newRegistry)
        );
        ...
        assertEq(
            ...
            newRegistry.getBalance(linkUpkeepID) +
                newRegistry.getBalance(linkUpkeepID2)
        );
        assertEq(
            newRegistry.getReserveAmount(address(usdToken18)),
            newRegistry.getBalance(usdUpkeepID18)
        );
        assertEq(
            newRegistry.getReserveAmount(address(weth)),
            newRegistry.getBalance(nativeUpkeepID)
        );
        assertEq(
            newRegistry.getReserveAmount(address(linkToken)),
            prevReserveBalances[0] -
        ...
        assertEq(
            usdToken18.balanceOf(address(newRegistry)),
            newRegistry.getBalance(usdUpkeepID18)
        );
        assertEq(
            weth.balanceOf(address(newRegistry)),
            newRegistry.getBalance(nativeUpkeepID)
        );
        assertEq(
            linkToken.balanceOf(address(registry)),
            prevTokenBalances[0] - linkToken.balanceOf(address(newRegistry))
        );
        assertEq(
            usdToken18.balanceOf(address(registry)),
        ...
        assertEq(
            weth.balanceOf(address(registry)),
            prevTokenBalances[2] - weth.balanceOf(address(newRegistry))
        );

        // assert upkeep data matches
        assertEq(
            prevUpkeepData[0],
            abi.encode(newRegistry.getUpkeep(linkUpkeepID))
        );
        assertEq(
            prevUpkeepData[1],
            abi.encode(newRegistry.getUpkeep(linkUpkeepID2))
        );
        assertEq(
            prevUpkeepData[2],
            abi.encode(newRegistry.getUpkeep(usdUpkeepID18))
        );
        assertEq(
            prevUpkeepData[3],
            abi.encode(newRegistry.getUpkeep(nativeUpkeepID))
        );
        assertEq(
            prevUpkeepTriggerData[0],
            newRegistry.getUpkeepTriggerConfig(linkUpkeepID)
        );
        assertEq(
            prevUpkeepTriggerData[1],
            newRegistry.getUpkeepTriggerConfig(linkUpkeepID2)
        );
        assertEq(
            prevUpkeepTriggerData[2],
            newRegistry.getUpkeepTriggerConfig(usdUpkeepID18)
        );
        assertEq(
            prevUpkeepTriggerData[3],
            newRegistry.getUpkeepTriggerConfig(nativeUpkeepID)
        );
...
contract Pause is SetUp {
    ...
    function test_CalledByOwner_success() external {
        ...
        (IAutomationV21PlusCommon.StateLegacy memory state, , , , ) = registry
            .getState();
    ...
    function test_revertsWhen_transmitInPausedRegistry() external {
        ...
        _transmitAndExpectRevert(
            usdUpkeepID18,
            registry,
            Registry.RegistryPaused.selector
        );
    }
}
...
contract Unpause is SetUp {
    ...
    function test_CalledByOwner_success() external {
        vm.startPrank(registry.owner());
        registry.pause();
        (IAutomationV21PlusCommon.StateLegacy memory state1, , , , ) = registry
            .getState();
        assertTrue(state1.paused);

        registry.unpause();
        (IAutomationV21PlusCommon.StateLegacy memory state2, , , , ) = registry
            .getState();
...
contract CancelUpkeep is SetUp {
    ...
    function test_RevertsWhen_UpkeepAlreadyCanceledByAdmin_CalledByOwner()
        external
    {
    ...
    function test_RevertsWhen_UpkeepAlreadyCanceledByOwner_CalledByAdmin()
        external
    {
        uint256 bn = block.number;
        vm.startPrank(registry.owner());
    ...
    function test_RevertsWhen_UpkeepAlreadyCanceledByAdmin_CalledByAdmin()
        external
    {
        uint256 bn = block.number;
        vm.startPrank(UPKEEP_ADMIN);
    ...
    function test_RevertsWhen_UpkeepAlreadyCanceledByOwner_CalledByOwner()
        external
    {
        uint256 bn = block.number;
        vm.startPrank(registry.owner());
    ...
    function test_CancelUpkeep_SetMaxValidBlockNumber_CalledByAdmin() external {
        ...
        uint256 maxValidBlocknumber = uint256(
            registry.getUpkeep(linkUpkeepID).maxValidBlocknumber
        );
...
contract TransferUpkeepAdmin is SetUp {
    ...
    function test_CancelTransfer_ByTransferToEmptyAddress() external {
        ...
        vm.expectEmit();
        emit UpkeepAdminTransferRequested(
            linkUpkeepID,
            UPKEEP_ADMIN,
            address(0)
        );
...
contract SetPayees is SetUp {
    ...
    function test_SetPayees_WhenExistingPayeesAreEmpty() external {
        ...
        for (uint256 i = 0; i < TRANSMITTERS.length; i++) {
            (, , , , address payee) = registry.getTransmitterInfo(
                TRANSMITTERS[i]
            );
        ...
        registry.setPayees(PAYEES);
        for (uint256 i = 0; i < TRANSMITTERS.length; i++) {
            (bool active, , , , address payee) = registry.getTransmitterInfo(
                TRANSMITTERS[i]
            );
            assertTrue(active);
