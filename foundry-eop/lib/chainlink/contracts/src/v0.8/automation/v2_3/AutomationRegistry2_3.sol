 ...
 */
contract AutomationRegistry2_3 is
    ...
    Chainable
{
    ...
    constructor(
        ...
        AutomationRegistryBase2_3(
            ...
            AutomationRegistryLogicC2_3(address(logicA))
                .getLinkUSDFeedAddress(),
            AutomationRegistryLogicC2_3(address(logicA))
                .getNativeUSDFeedAddress(),
            AutomationRegistryLogicC2_3(address(logicA))
                .getFastGasFeedAddress(),
            AutomationRegistryLogicC2_3(address(logicA))
    ...
    function _handleReport(
        ...
        UpkeepTransmitInfo[]
            memory upkeepTransmitInfo = new UpkeepTransmitInfo[](
                report.upkeepIds.length
            );
        ...
        for (uint256 i = 0; i < report.upkeepIds.length; i++) {
            ...
            upkeepTransmitInfo[i].triggerType = _getTriggerType(
                report.upkeepIds[i]
            );

            (
                upkeepTransmitInfo[i].earlyChecksPassed,
                upkeepTransmitInfo[i].dedupID
            ) = _prePerformChecks(
                report.upkeepIds[i],
                blocknumber,
                report.triggers[i],
            ...
            transmitVars.totalCalldataWeight += upkeepTransmitInfo[i]
                .calldataWeight;
        ...
        gasOverhead =
            gasOverhead /
            transmitVars.numUpkeepsPassedChecks +
            ACCOUNTING_PER_UPKEEP_GAS_OVERHEAD;
                ...
                if (upkeepTransmitInfo[i].earlyChecksPassed) {
                    ...
                    PaymentReceipt memory receipt = _handlePayment(
                        ...
                        PaymentParams({
                            ...
                            nativeUSD: nativeUSD,
                            billingToken: upkeepTransmitInfo[i]
                                .upkeep
                                .billingToken,
                    ...
                    transmitVars.totalPremium += receipt.premiumETHuels;
                    transmitVars.totalReimbursement += receipt
                        .gasReimbursementETHuels;

                    emit UpkeepPerformed(
                        report.upkeepIds[i],
                        upkeepTransmitInfo[i].performSuccess,
                        receipt.gasChargeInBillingToken +
        ...
        s_reserveAmounts[IERC20(address(i_link))] +=
            transmitVars.totalReimbursement +
            transmitVars.totalPremium;
    }
    ...
    function setConfigTypeSafe(
        ...
        if (billingTokens.length != billingConfigs.length)
            revert ParameterLengthError();
        ...
        uint32 newLatestConfigBlockNumber = uint32(
            onchainConfig.chainModule.blockNumber()
        );
