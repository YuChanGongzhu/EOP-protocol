...
abstract contract AutomationRegistryBase2_3 is ConfirmedOwner {
    ...
    address internal constant IGNORE_ADDRESS =
    ...
    bytes4 internal constant CHECK_CALLBACK_SELECTOR =
        StreamsLookupCompatibleInterface.checkCallback.selector;
    ...
    mapping(address => MigrationPermission)
        internal s_peerRegistryMigrationPermission; // Permissions for migration to and fro
    ...
    // billing
    mapping(IERC20 billingToken => uint256 reserveAmount)
        internal s_reserveAmounts; // unspent user deposits + unwithdrawn NOP payments
    mapping(IERC20 billingToken => BillingConfig billingConfig)
        internal s_billingConfigs; // billing configurations for different tokens
    mapping(uint256 upkeepID => BillingOverrides billingOverrides)
        internal s_billingOverrides; // billing overrides for specific upkeeps
     ...
     * @member gasReimbursementETHuels the amount to reimburse a node for gas spent
     * @member premiumETHuels the premium paid to NOPs, shared between all nodes
     */
    // solhint-disable-next-line gas-struct-packing
    ...
    function _createUpkeep(
        ...
        if (checkData.length > s_storage.maxCheckDataSize)
            revert CheckDataExceedsLimit();
        if (
            upkeep.performGas < PERFORM_GAS_MIN ||
            upkeep.performGas > s_storage.maxPerformGas
        ) revert GasLimitOutsideRange();
        if (address(s_upkeep[id].forwarder) != address(0))
            revert UpkeepAlreadyExists();
        if (
            address(s_billingConfigs[upkeep.billingToken].priceFeed) ==
            address(0)
        ) revert InvalidToken();
        s_upkeep[id] = upkeep;
        s_upkeepAdmin[id] = admin;
        s_checkData[id] = checkData;
        s_reserveAmounts[upkeep.billingToken] =
            s_reserveAmounts[upkeep.billingToken] +
            upkeep.balance;
    ...
    function _createID(Trigger triggerType) internal view returns (uint256) {
        ...
        bytes memory idBytes = abi.encodePacked(
            keccak256(
                abi.encode(
                    chainModule.blockHash((chainModule.blockNumber() - 1)),
                    address(this),
                    s_storage.nonce
                )
            )
    ...
    function _getNativeUSD(
        ...
        (, int256 feedValue, , uint256 timestamp, ) = i_nativeUSDFeed
            .latestRoundData();
        if (
            feedValue <= 0 ||
            block.timestamp < timestamp ||
            (hotVars.stalenessSeconds > 0 &&
                hotVars.stalenessSeconds < block.timestamp - timestamp)
        ) {
    ...
    function _getBillingTokenPaymentParams(
        ...
        (, int256 feedValue, , uint256 timestamp, ) = config
            .priceFeed
            .latestRoundData();
        if (
            feedValue <= 0 ||
    ...
    function _calculatePaymentAmount(
        ...
        uint256 gasWei = paymentParams.fastGasWei *
        ...
        uint256 numeratorScalingFactor = decimals > 18
            ? 10 ** (decimals - 18)
            : 1;
        uint256 denominatorScalingFactor = decimals < 18
            ? 10 ** (18 - decimals)
            : 1;
        ...
        receipt.gasChargeInBillingToken = SafeCast.toUint96(
            ((gasPaymentHexaicosaUSD * numeratorScalingFactor) +
                (paymentParams.billingTokenParams.priceUSD *
                    denominatorScalingFactor -
                    1)) /
                (paymentParams.billingTokenParams.priceUSD *
                    denominatorScalingFactor)
        );
        // 18 decimals: 26 decimals / 8 decimals
        receipt.gasReimbursementETHuels = SafeCast.toUint96(
            gasPaymentHexaicosaUSD / paymentParams.linkUSD
        );

        // premium calculation
        uint256 flatFeeHexaicosaUSD = uint256(
            paymentParams.billingTokenParams.flatFeeMilliCents
        ) * 1e21; // 1e13 for milliCents to attoUSD and 1e8 for attoUSD to hexaicosaUSD
        uint256 premiumHexaicosaUSD = ((((gasWei * paymentParams.gasLimit) +
            paymentParams.l1CostWei) *
        ...
        receipt.premiumETHuels = SafeCast.toUint96(
            premiumHexaicosaUSD / paymentParams.linkUSD
        );
        ...
        receipt.billingUSD = SafeCast.toUint96(
            paymentParams.billingTokenParams.priceUSD
        );
    ...
    function _getMaxPayment(
        ...
        BillingTokenPaymentParams
            memory paymentParams = _getBillingTokenPaymentParams(
                hotVars,
                billingToken
            );
        if (s_upkeep[upkeepId].overridesEnabled) {
            BillingOverrides memory billingOverrides = s_billingOverrides[
                upkeepId
            ];
            // use the overridden configs
            paymentParams.gasFeePPB = billingOverrides.gasFeePPB;
            paymentParams.flatFeeMilliCents = billingOverrides
                .flatFeeMilliCents;
        }
    ...
    function _checkPayload(
        ...
        if (triggerType == Trigger.CONDITION) {
            return
                abi.encodeWithSelector(CHECK_SELECTOR, s_checkData[upkeepId]);
        } else if (triggerType == Trigger.LOG) {
    ...
    function _prePerformChecks(
        ...
        if (transmitInfo.triggerType == Trigger.CONDITION) {
            ...
            bool valid;
            (valid, dedupID) = _validateLogTrigger(
                upkeepId,
                blocknumber,
                rawTrigger,
                hotVars
    ...
    function _validateConditionalTrigger(
        ...
        ConditionalTrigger memory trigger = abi.decode(
            rawTrigger,
            (ConditionalTrigger)
        );
        ...
        if (
            (hotVars.reorgProtectionEnabled &&
                (trigger.blockHash != bytes32("") &&
                    hotVars.chainModule.blockHash(trigger.blockNum) !=
                    trigger.blockHash)) || trigger.blockNum >= blocknumber
        ) {
    ...
    function _validateLogTrigger(
        ...
        bytes32 dedupID = keccak256(
            abi.encodePacked(
                upkeepId,
                trigger.logBlockHash,
                trigger.txHash,
                trigger.logIndex
            )
        );
    ...
    function _handlePayment(
        ...
        if (upkeep.overridesEnabled) {
            ...
            // use the overridden configs
            paymentParams.billingTokenParams.gasFeePPB = billingOverrides
                .gasFeePPB;
            paymentParams
                .billingTokenParams
                .flatFeeMilliCents = billingOverrides.flatFeeMilliCents;
        }
        ...
        uint96 payment = receipt.gasChargeInBillingToken +
            receipt.premiumInBillingToken;
        ...
        if (balance < receipt.gasChargeInBillingToken) {
            ...
            receipt.gasReimbursementETHuels = SafeCast.toUint96(
                (balance *
                    paymentParams.billingTokenParams.priceUSD *
                    scalingFactor1) / (paymentParams.linkUSD * scalingFactor2)
            );
            receipt.premiumETHuels = 0;
            ...
            receipt.premiumETHuels = SafeCast.toUint96(
                ((balance *
                    ...
                    receipt.gasReimbursementETHuels
            );
            // round up
            receipt.premiumInBillingToken = SafeCast.toUint96(
                ((receipt.premiumETHuels *
                    paymentParams.linkUSD *
                    scalingFactor2) +
                    (paymentParams.billingTokenParams.priceUSD *
                        scalingFactor1 -
                        1)) /
    ...
    function _requireAdminAndNotCancelled(uint256 upkeepId) internal view {
        ...
        if (s_upkeep[upkeepId].maxValidBlocknumber != UINT32_MAX)
    ...
    function _setBillingConfig(
        ...
        for (uint256 i = 0; i < billingTokens.length; i++) {
            ...
            if (
                config.decimals != token.decimals() ||
                config.priceFeed.decimals() != 8
            ) {
                revert InvalidToken();
            }
            ...
            if (
                address(token) == address(i_link) &&
                mode == PayoutMode.OFF_CHAIN
            ) {
                revert InvalidToken();
            }
            if (
                address(token) == ZERO_ADDRESS ||
                address(config.priceFeed) == ZERO_ADDRESS
            ) {
    ...
    function _updateTransmitters(
        ...
        for (uint256 i = 0; i < s_transmittersList.length; i++) {
            ...
            if (
                mode == PayoutMode.OFF_CHAIN &&
                s_transmitters[transmitterAddress].balance > 0
            ) {
   ...
   */
    function _linkAvailableForPayment() internal view returns (int256) {
        return
            int256(i_link.balanceOf(address(this))) -
            int256(s_reserveAmounts[IERC20(address(i_link))]);
    }
}
