...
type PaymentChecker struct {
	...
	METHobPayment *assets.Link
}

// NewPaymentChecker constructs a new payment checker
func NewPaymentChecker(minContractPayment, mETHobPayment *assets.Link) *PaymentChecker {
	return &PaymentChecker{
		MinContractPayment: minContractPayment,
		METHobPayment:      mETHobPayment,
	}
}
...
func (c *PaymentChecker) SufficientPayment(payment *big.Int) bool {
	...
	aboveOrEqMETHobPayment := true
	if c.METHobPayment != nil {
		aboveOrEqMETHobPayment = payment.Cmp(c.METHobPayment.ToInt()) >= 0
	}
	return aboveOrEqMinGlobalPayment && aboveOrEqMETHobPayment
}
