...
func TestPaymentChecker_SufficientPayment(t *testing.T) {
	...
	testCases := []struct {
		...
		mETHobPayment      interface{} // nil or int64
		want               bool
	}{
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			var mETHobPayment *assets.Link
			if tc.mETHobPayment != nil {
				mjb := assets.Link(*big.NewInt(tc.mETHobPayment.(int64)))
				mETHobPayment = &mjb
			}

			checker := fluxmonitorv2.NewPaymentChecker(assets.NewLinkFromJuels(tc.minContractPayment), mETHobPayment)
