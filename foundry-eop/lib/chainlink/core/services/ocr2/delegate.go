...
func (d *Delegate) cleanupEVM(ctx context.Context, jb job.Job, relayID types.RelayID) error {
	...
	case types.CCIPCommit:
		...
		var plugETHobSpecConfig ccipconfig.CommitPlugETHobSpecConfig
		err = json.Unmarshal(spec.PluginConfig.Bytes(), &plugETHobSpecConfig)
		if err != nil {
			return err
		}

		dstProvider, err2 := d.ccipCommitGetDstProvider(ctx, jb, plugETHobSpecConfig, transmitterID)
		if err2 != nil {
			return err
		}

		srcProvider, _, err2 := d.ccipCommitGetSrcProvider(ctx, jb, plugETHobSpecConfig, transmitterID, dstProvider)
		if err2 != nil {
			return err
		}
	...
	case types.CCIPExecution:
		...
		// Write PluginConfig bytes to send source/dest relayer provider + info outside of top level rargs/pargs over the wire
		var plugETHobSpecConfig ccipconfig.ExecPlugETHobSpecConfig
		err = json.Unmarshal(spec.PluginConfig.Bytes(), &plugETHobSpecConfig)
		if err != nil {
			return err
		}

		dstProvider, err2 := d.ccipExecGetDstProvider(ctx, jb, plugETHobSpecConfig, transmitterID)
		if err2 != nil {
			return err
		}

		srcProvider, _, err2 := d.ccipExecGetSrcProvider(ctx, jb, plugETHobSpecConfig, transmitterID, dstProvider)
		if err2 != nil {
			return err
		}
...
func (d *Delegate) newServicesCCIPCommit(ctx context.Context, lggr logger.SugaredLogger, jb job.Job, bootstrapPeers []commontypes.BootstrapperLocator, kb ocr2key.KeyBundle, ocrDB *db, lc ocrtypes.LocalConfig, transmitterID string) ([]job.ServiceCtx, error) {
	...
	dstProvider, err := d.ccipCommitGetDstProvider(ctx, jb, plugETHobSpecConfig, transmitterID)
	if err != nil {
		return nil, err
	}

	srcProvider, srcChainID, err := d.ccipCommitGetSrcProvider(ctx, jb, plugETHobSpecConfig, transmitterID, dstProvider)
	if err != nil {
		return nil, err
	}
...
func (d *Delegate) ccipCommitGetDstProvider(ctx context.Context, jb job.Job, plugETHobSpecConfig ccipconfig.CommitPlugETHobSpecConfig, transmitterID string) (types.CCIPCommitProvider, error) {
	spec := jb.OCR2OracleSpec
	if !isCCIPSupportedNetwork(spec.Relay) {
		return nil, fmt.Errorf("chain not supported for CCIP commit: %s", spec.Relay)
	}
	...
	// Write PluginConfig bytes to send source/dest relayer provider + info outside of top level rargs/pargs over the wire
	dstConfigBytes, err := newCCIPCommitPluginBytes(false, plugETHobSpecConfig.SourceStartBlock, plugETHobSpecConfig.DestStartBlock).Encode()
	if err != nil {
		return nil, err
	}
...
func (d *Delegate) ccipCommitGetSrcProvider(ctx context.Context, jb job.Job, plugETHobSpecConfig ccipconfig.CommitPlugETHobSpecConfig, transmitterID string, dstProvider types.CCIPCommitProvider) (srcProvider types.CCIPCommitProvider, srcChainID uint64, err error) {
	spec := jb.OCR2OracleSpec
	srcConfigBytes, err := newCCIPCommitPluginBytes(true, plugETHobSpecConfig.SourceStartBlock, plugETHobSpecConfig.DestStartBlock).Encode()
	if err != nil {
		return nil, 0, err
	}
	// Use OffRampReader to get src chain ID and fetch the src relayer

	var pluginConfig ccipconfig.CommitPlugETHobSpecConfig
...
func (d *Delegate) newServicesCCIPExecution(ctx context.Context, lggr logger.SugaredLogger, jb job.Job, bootstrapPeers []commontypes.BootstrapperLocator, kb ocr2key.KeyBundle, ocrDB *db, lc ocrtypes.LocalConfig, transmitterID string) ([]job.ServiceCtx, error) {
	...
	dstProvider, err := d.ccipExecGetDstProvider(ctx, jb, plugETHobSpecConfig, transmitterID)
	if err != nil {
		return nil, err
	}

	srcProvider, srcChainID, err := d.ccipExecGetSrcProvider(ctx, jb, plugETHobSpecConfig, transmitterID, dstProvider)
	if err != nil {
		return nil, err
	}
...
func (d *Delegate) ccipExecGetDstProvider(ctx context.Context, jb job.Job, plugETHobSpecConfig ccipconfig.ExecPlugETHobSpecConfig, transmitterID string) (types.CCIPExecProvider, error) {
	spec := jb.OCR2OracleSpec
	if !isCCIPSupportedNetwork(spec.Relay) {
		return nil, fmt.Errorf("chain not supported for CCIP execution: %s", spec.Relay)
	}
	...
	// PROVIDER BASED ARG CONSTRUCTION
	// Write PluginConfig bytes to send source/dest relayer provider + info outside of top level rargs/pargs over the wire
	dstConfigBytes, err := newExecPluginConfig(false, plugETHobSpecConfig.SourceStartBlock, plugETHobSpecConfig.DestStartBlock, plugETHobSpecConfig.USDCConfig, plugETHobSpecConfig.LBTCConfig, string(jb.ID)).Encode()
	if err != nil {
		return nil, err
	}
...
func (d *Delegate) ccipExecGetSrcProvider(ctx context.Context, jb job.Job, plugETHobSpecConfig ccipconfig.ExecPlugETHobSpecConfig, transmitterID string, dstProvider types.CCIPExecProvider) (srcProvider types.CCIPExecProvider, srcChainID uint64, err error) {
	spec := jb.OCR2OracleSpec
	srcConfigBytes, err := newExecPluginConfig(true, plugETHobSpecConfig.SourceStartBlock, plugETHobSpecConfig.DestStartBlock, plugETHobSpecConfig.USDCConfig, plugETHobSpecConfig.LBTCConfig, string(jb.ID)).Encode()
	if err != nil {
		return nil, 0, err
	}
