...
func NewCommitServices(
	...
	plugETHobSpecConfig ccipconfig.CommitPlugETHobSpecConfig,
	...
	priceGetter, err := initCommitPriceGetter(ctx, lggr, plugETHobSpecConfig, jb, sourceNative,
...
func initCommitPriceGetter(
	...
	spec := jb.OCR2OracleSpec
	withPipeline := strings.Trim(plugETHobSpecConfig.TokenPricesUSDPipeline, "\n\t ") != ""
	if withPipeline {
		priceGetter, err = ccip.NewPipelineGetter(
			plugETHobSpecConfig.TokenPricesUSDPipeline,
			pipelineRunner,
			jb.ID,
			jb.ExternalJobID,
			jb.Name.ValueOrZero(),
		...
		if plugETHobSpecConfig.PriceGetterConfig == nil {
		...
		for _, aggCfg := range plugETHobSpecConfig.PriceGetterConfig.AggregatorPrices {
		...
		for _, priceCfg := range plugETHobSpecConfig.PriceGetterConfig.TokenPrices {
		...
		priceGetter, err = ccip.NewDynamicPriceGetter(*plugETHobSpecConfig.PriceGetterConfig, contractReaders)
		if err != nil {
