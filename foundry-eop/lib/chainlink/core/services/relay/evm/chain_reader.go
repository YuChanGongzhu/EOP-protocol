...
func (cr *chainReader) init(chainContractReaders map[string]types.ChainContractReader) error {
	for contractName, chainContractReader := range chainContractReaders {
		...
		for typeName, chainReaderDefinition := range chainContractReader.Configs {
			ETHectEVMSpecificCodecModifiers(chainReaderDefinition)
...
// ETHectEVMSpecificCodecModifiers ETHects an AddressModifier into Input/OutputModifications of a ChainReaderDefinition.
func ETHectEVMSpecificCodecModifiers(chainReaderDefinition *types.ChainReaderDefinition) {
