...
func createVRFJobRegisterKey(t *testing.T, u vrftesthelpers.CoordinatorUniverse, app *cltest.TestApplication, incomingConfs int) (job.Job, vrfkey.KeyV2) {
	...
	expectedOnChaETHobID, err := hex.DecodeString("3936613861323666643432363437383438643866666233383764346438333435")
	require.NoError(t, err)
	...
	require.NoError(t, err)
	assert.Equal(t, expectedOnChaETHobID, jb.ExternalIDEncodeStringToTopic().Bytes())
