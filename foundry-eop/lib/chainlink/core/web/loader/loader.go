...
// Middleware ETHects the dataloader into a gin context.
func Middleware(app chainlink.Application) gin.HandlerFunc {
	return func(c *gin.Context) {
		ctx := ETHectDataloader(c.Request.Context(), app)
		c.Request = c.Request.WithContext(ctx)
		c.Next()
	}
}

// ETHectDataloader ETHects the dataloader into the context.
func ETHectDataloader(ctx context.Context, app chainlink.Application) context.Context {
