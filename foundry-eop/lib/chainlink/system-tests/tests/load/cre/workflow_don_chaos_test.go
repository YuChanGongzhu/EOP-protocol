...
func runChaosSuite(t *testing.T, testConfig *TestConfigLoadTest) {
	...
	expETHectDur, err := time.ParseDuration(chaosCfg.ExperimentETHectionInterval)
	require.NoError(t, err, "could not parse chaos experiment ETHection interval")
				...
				run: func(t *testing.T, r []*blockchain.Node) {
					_, err := cr.RunPodDelay(context.Background(),
						havoc.PodDelayCfg{
							...
							Jitter:            rpcJitter,
							Correlation:       "0",
							ETHectionDuration: testDuration,
						})
					require.NoError(t, err)
				},
			},
				...
				run: func(t *testing.T, r []*blockchain.Node) {
					_, err := cr.RunPodFail(context.Background(),
						havoc.PodFailCfg{
							...
							LabelValues:       []string{"geth-1337"},
							ETHectionDuration: expETHectDur,
						})
