...
contract CatNFT is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    uint256 public drawFee = 0.001 ether; // 抽卡费用 (0.001 ETH)
    ...
    // 简易元交易 Nonce
    mapping(address => uint256) public nonces; // 用户钱包 -> 下一个可用 nonce
    ...
    /**
     * @dev 使用签名的社交互动（由授权代付者发起）
     */
    function socialInteractionWithSig(
        string memory myNFC,
        ...
        bytes32 messageHash = keccak256(
            abi.encodePacked(address(this), "socialInteraction", wallet, myNFC, otherNFC, nonce, deadline)
        );
        require(_verifySignature(wallet, messageHash, signature), "Bad signature");
        nonces[wallet]++;

        require(bytes(myNFC).length > 0 && bytes(otherNFC).length > 0, "Invalid NFC UID");
        require(keccak256(bytes(myNFC)) != keccak256(bytes(otherNFC)), "Cannot interact with yourself");
        require(nfcRegistry.isNFCBound(myNFC) && nfcRegistry.isNFCBound(otherNFC), "NFC not registered");
        INFCWalletRegistry.NFCBinding memory myBinding = nfcRegistry.getNFCBinding(myNFC);
        require(wallet == myBinding.walletAddress, "You don't own this NFC");
        require(!socialInteractions[myNFC][otherNFC], "Already interacted with this NFC");

        socialInteractions[myNFC][otherNFC] = true;
        socialInteractions[otherNFC][myNFC] = true;
        interactedNFCs[myNFC].push(otherNFC);
        interactedNFCs[otherNFC].push(myNFC);

        drawCounts[myNFC] += socialInteractionReward;
        drawCounts[otherNFC] += socialInteractionReward;

        emit SocialInteractionCompleted(myNFC, otherNFC, socialInteractionReward, drawCounts[myNFC]);
        emit SocialInteractionCompleted(otherNFC, myNFC, socialInteractionReward, drawCounts[otherNFC]);
    }
    ...
    function socialInteraction(
        ...
        INFCWalletRegistry.NFCBinding memory myBinding = nfcRegistry
            .getNFCBinding(myNFC);
        require(msg.sender == myBinding.walletAddress, "You don't own this NFC");
    ...
    /**
     * @dev 使用签名与代付抽卡（由授权代付者发起，需附带 drawFee）
     */
    function drawCatNFTWithTicketsWithSig(
        string memory nfcUID,
        string memory catName,
        address wallet,
        uint256 deadline,
        ...
        require(nonces[wallet] == nonce, "Invalid nonce");
        bytes32 messageHash = keccak256(
            abi.encodePacked(address(this), "drawCatNFTWithTickets", wallet, nfcUID, catName, nonce, deadline)
        );
        require(_verifySignature(wallet, messageHash, signature), "Bad signature");
        nonces[wallet]++;
        ...
        require(drawCounts[nfcUID] > 0, "No draw tickets available");
        require(nfcRegistry.isNFCBound(nfcUID), "NFC not registered");
        INFCWalletRegistry.NFCBinding memory binding = nfcRegistry.getNFCBinding(nfcUID);
        require(wallet == binding.walletAddress, "You don't own this NFC");
        require(userCats[binding.walletAddress].length < MAX_CATS_PER_USER, "Too many cats");
        ...
        if (rarity == CatRarity.UR) {
            rainbowCatMinted = true;
            rainbowCatTokenId = newTokenId;
            emit RainbowCatFound(newTokenId, catName, binding.walletAddress, block.timestamp);
        }

        userCats[binding.walletAddress].push(newTokenId);
        _safeMint(binding.walletAddress, newTokenId);

        emit CatDrawnWithTickets(newTokenId, catName, rarity, color, binding.walletAddress, nfcUID, drawCounts[nfcUID]);
        emit CatNFTMinted(newTokenId, catName, rarity, color, binding.walletAddress, block.timestamp);
    }
