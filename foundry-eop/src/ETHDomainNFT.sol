 ...
 */
contract ETHDomainNFT is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard {
    using Strings for uint256;

    // 域名信息结构
    struct DomainInfo {
        string domainName; // 完整域名 (例如: alice.ETH)
        address owner; // 域名所有者
    ...
    ) ERC721("ETH Domain NFT", "ETHDN") Ownable(msg.sender) {
    ...
    function mintDomainNFT(
        ...
        string memory fullDomain = string(
            abi.encodePacked(domainPrefix, ".ETH")
        );
